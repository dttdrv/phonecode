...
2. On **Setup**, tap **Connect a model**. Do not tap **Skip setup for now**: without a configured
   provider that action does not complete onboarding in the current release candidate.
3. On **Providers**, open the built-in API-key provider named in the private Play Console
   instructions and enter the dedicated key in **API key**. The provider row must then show **Key set**.
4. Use Back to return to **Providers**, then Back again to **Setup**, and tap **Continue to
   PhoneCode**. PhoneCode activates an available model for the configured provider; the private
   instructions must name the exact model verified for review.
5. Create a disposable project folder using Android's system folder picker.
6. Start a new chat and use this prompt:
...
7. Approve the file action if the chosen permission mode asks. Verify the response and file content.
8. Open the project drawer to show the project-scoped chat, then open Settings to inspect Skills and
   MCP pages. MCP setup is optional and not required for the test.
9. Open the action menu on an AI response and confirm **Report AI response** is available in-app.
