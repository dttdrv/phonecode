...
If you choose Send safety feedback and tap Send, PhoneCode sends the selected category, an optional note you write, the app version, and the platform to dttdrv.xyz. It never attaches the AI response, your prompt, files, credentials, tool activity, chat history, provider or model identifiers, or a device identifier. Feedback submissions are stored in a private Cloudflare D1 database and are used to investigate harmful output and improve filtering and moderation. They are scheduled for deletion after 89 days; daily and request-triggered cleanup is designed to keep retention within 90 days.
