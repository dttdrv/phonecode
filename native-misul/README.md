...
Before building the current PhoneCode runtime, apply `native-misul/patches/provider-failure-rpc.patch`, `native-misul/patches/android-https-transport.patch`, and `native-misul/patches/openrouter-final-usage.patch` to the Misul source, in that order. Apply the zero-context transport patch with `git apply --unidiff-zero`. The first patch preserves Misul's structured, redacted provider failure in the Android RPC settlement. The second uses Android's dual-stack resolver, races the returned addresses, and eagerly loads Android's system CA bundle before native HTTPS, avoiding Zig's Linux resolver path inside the app process without narrowing custom providers to IPv4. The third accepts OpenRouter's final usage event when it repeats a completed choice with an empty delta; any new content after the finish reason is still rejected.
